...
const ecdsaTest = async function (m) {
  ...
  const sig = await subtle.sign({ name: 'ECDSA', hash: { name: 'SHA-256' } }, ecdsaKey.privateKey, Buffer.from(m))
  console.timeEnd('ECDSA Signing Total Time')

  console.time('ECDSA Verify Total Time')
  const ret = await subtle.verify({ name: 'ECDSA', hash: { name: 'SHA-256' } }, ecdsaKey.publicKey, sig, Buffer.from(m))
...
const rsaTest = async function (m) {
  ...
  const sig = await subtle.sign({ name: 'RSA-PSS', saltLength: 32 }, rsaKey.privateKey, Buffer.from(m))
  console.timeEnd('RSA Signing Total Time')

  console.time('RSA Verify Total Time')
  const ret = await subtle.verify({ name: 'RSA-PSS', saltLength: 32 }, rsaKey.publicKey, sig, Buffer.from(m))
